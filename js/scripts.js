	...
	$(document).ready(function() {
		...
		$('.tour-view-type').on('click', function(e) {
				...
				if($(e.target).hasClass('tour-list-icon') || $(e.target).is('.tour-list-icon i')) {
					...
					$('.tours .column').removeClass('col-lg-6').addClass('col-12');
				...
				if($(e.target).hasClass('tour-grid-icon') || $(e.target).is('.tour-grid-icon i')) {
					...
					$('.tours .column').addClass('col-lg-6').removeClass('col-12');
		...
		// SLIDER
		var menu = ['WISATA UNIK DI<span>SAMARINDA</span>', 'LESTARIKAN<span>SUNGAI MAHAKAM</span>', 'SENSASI<span>YANG BERBEDA</span>'];
